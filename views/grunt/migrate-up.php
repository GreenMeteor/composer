...
<div id="migrate-up" class="panel panel-default">
    ...
    <div class="panel-body" data-ui-widget="pjax-container">
        ...
        <div class="form-group">
            <?= $form->field($model, 'themeName')->textInput(['placeholder' => 'Enter theme name'])->label('Theme Name (optional)') ?>
        </div>
...
$this->registerJs('
    $(document).on("submit", "#migrate-up-form", function(event) {
        ...
        $.ajax({
            type: "POST",
            url: "' . Yii::$app->urlManager->createUrl(['/composer/grunt/migrate-up']) . '",
            data: formData,
            success: function(response) {
                if (response.success) {
                    ...
                    $("#output-container").html("<h2>Error</h2><pre>" + Html::encode(response.error) + "</pre>");
                }
            },
            error: function(xhr, status, error) {
                $("#output-container").html("<h2>Error</h2><pre>" + Html::encode(error) + "</pre>");
            },
            complete: function() {
                $btn.button("reset");
            }
